...
const TraversalEdgeBehavior kDefaultRouteTraversalEdgeBehavior =
...
class _RouteEntry extends RouteTransitionRecord {
  ...
  WeakReference<_RoutePlaceholder> lastAnnouncedPoppedNextRoute =
      WeakReference<_RoutePlaceholder>(
          notAnnounced); // last argument to Route.didPopNext
  _RoutePlaceholder? lastAnnouncedNextRoute =
        ...
        switch (defaultTargetPlatform) {
          case TargetPlatform.android:
            ...
            SystemChannels.accessibility
                .send(const FocusSemanticEvent().toMap(nodeId: reFocusNode));
          case TargetPlatform.iOS:
            SystemChannels.accessibility
                .send(const FocusSemanticEvent().toMap(nodeId: lastFocusNode));
          case _:
            break;
        }
      });
...
class NavigatorState extends State<Navigator>
  ...
  void _recordLastFocus() {
    final _RouteEntry? entry =
        _history.where(_RouteEntry.isPresentPredicate).lastOrNull;
  ...
  void dispose() {
    ...
    ServicesBinding.instance.accessibilityFocus
        .removeListener(_recordLastFocus);
...
class _AnonymousRestorationInformation extends _RestorationInformation {
  ...
  _AnonymousRestorationInformation.fromSerializableData(List<Object?> data)
      : assert(data.length > 1),
        restorationScopeId = data[0]! as int,
        routeBuilder = ui.PluginUtilities.getCallbackFromHandle(
